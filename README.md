...
此文件中的班级名如`C5G6`将作为文件`统计学生作业.bat`的第2个参数
...
:heavy_exclamation_mark:温馨提示
- 此文件中的每行如`Lessson1`将作为文件`统计学生作业.bat`的第1个参数
- `统计学生作业.bat` 运行时，是依据学生姓名进行查找的，所以学生提交的作业文件的文件名必须包含该学生的姓名
...
```
### 生成统计名单
打开cmd窗口，进入`统计学生作业.bat`所在的目录。
如果`统计学生作业.bat`文件在E盘，可以输入如下命令：
```
cd E: /d
```
在窗口中运行`统计学生作业 %1 %2`命令，参数%1即指课时，要与文件`A2课时清单.txt`里的课时名称保持一致。如`Lesson1`
